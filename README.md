...
<div align="center">
    ...
    <p>!  <strong><a href="#%E6%8B%93%E5%B1%95%E5%88%97%E8%A1%A8"> 点击前往 -> 拓展列表 </a></strong> !</p>
</div>
...
```yaml
    ...
    - EXT_NAME: ext_random  # 拓展文件名 (不含'.py')
      IS_ACTIVE: true   # 是否启用 (设为启用才会加载拓展，同时需要保证 NG_ENABLE_EXT 项开启)
      EXT_CONFIG:       # 拓展配置 如该拓展插件无要求可不写此项
...
#### > 主动记忆能力拓展模块 (因bot使用积极性不佳，暂移入备份，不推荐使用)
...
- 增加了一个新的语音拓展 `ext_VOICEVOX` 能够更便捷地实现本地部署 (感谢 @恋如雨止 提供技术支持)
