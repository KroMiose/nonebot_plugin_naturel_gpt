...
class CommandManager:
    ...
    def execute(self, chat: Chat, command: str, chat_presets_dict: dict) -> dict:
        ...
        if target_route:
            try:
                return self.command_router[target_route]["func"](
                    option_dict, param_dict, chat, chat_presets_dict
                )
    ...
    def submit_commands(self):
        ...
        self.command_router = dict(
            sorted(
                self.command_router.items(),
            ...
            try:
                if "/".join(cmd_list).startswith(route):
                    ...
                    if len(params_list) > 0:
                        ...
                        param_dict[params_list[-1]] = " ".join(
                            cmd_list[len(params_list) - 1 :]
                        )
                    target_route = route
                    break
...
def _(option_dict, param_dict, chat: Chat, chat_presets_dict: dict):
    presets_show_text = "\n".join(
        [
            f'  -> {k + " (当前)" if k == chat.get_chat_preset_key() else k}'
            for k in chat_presets_dict.keys()
        ]
    )
...
def _(option_dict, param_dict, chat: Chat, chat_presets_dict: dict):
    ...
    if target_preset_key not in chat_presets_dict:  # 如果预设不存在，匹配最相似的预设
        target_preset_key = difflib.get_close_matches(
            target_preset_key, chat_presets_dict.keys(), n=1, cutoff=0.3
        )
    ...
    if option_dict.get("global"):  # 全局应用
        ...
        for chat_key in global_chat_dict.keys():
            if not global_chat_dict[chat_key].change_presettings(target_preset_key):
                err_cnt += 1
        return {
            "msg": f"应用预设: {target_preset_key} (￣▽￣)-Completed! (所有会话) {f'错误计数:{err_cnt}' if err_cnt else ''}",
            "is_progress": True,
        }
        ...
        return {
            "msg": f"应用预设: {target_preset_key} (￣▽￣)-ok! (会话: {target_chat_key})",
            "is_progress": True,
        }
    else:  # 当前会话应用
...
def _(option_dict, param_dict, chat: Chat, chat_presets_dict: dict):
    ...
    if option_dict.get("global"):  # 全局应用
        success_cnt, fail_cnt = PersistentDataManager.instance.add_preset_for_all(
            preset_key=target_preset_key, bot_self_introl=bot_self_introl
        )
        return {
            "msg": f"添加预设: {target_preset_key} (￣▽￣)-ok! (所有会话) 成功:{success_cnt}，失败:{fail_cnt}",
            "is_progress": True,
        }
    elif option_dict.get("target"):  # 指定会话应用
        target_chat_key = option_dict.get("target")
        if target_chat_key not in global_chat_dict:
            return {"msg": f"会话: {target_chat_key} 不存在! (；′⌒`)"}
        if PersistentDataManager.instance.add_preset(
            chat_key=target_chat_key,
            preset_key=target_preset_key,
            bot_self_introl=bot_self_introl,
        ):
            return {
                "msg": f"添加预设: {target_preset_key} (￣▽￣)-ok! (会话: {target_chat_key})",
                "is_progress": True,
            }
        else:
            return {
                "msg": f"添加预设: {target_preset_key} 失败! (会话: {target_chat_key}) (；′⌒`)",
                "is_progress": True,
            }
    else:  # 当前会话应用
        if PersistentDataManager.instance.add_preset(
            chat_key=chat.get_chat_key(),
            preset_key=target_preset_key,
            bot_self_introl=bot_self_introl,
        ):
    ...
    if option_dict.get("global"):  # 全局应用
        if option_dict.get("global"):  # 全局应用
            (
                success_cnt,
                fail_cnt,
            ) = PersistentDataManager.instance.update_preset_for_all(
                preset_key=target_preset_key, bot_self_introl=bot_self_introl
            )
            return {
                "msg": f"编辑预设: {target_preset_key} (￣▽￣)-ok! (所有会话) 成功:{success_cnt}，失败:{fail_cnt}",
                "is_progress": True,
            }
    elif option_dict.get("target"):  # 指定会话应用
        target_chat_key = option_dict.get("target")
        if target_chat_key not in global_chat_dict:
            return {"msg": f"会话: {target_chat_key} 不存在! (；′⌒`)"}
        if PersistentDataManager.instance.update_preset(
            chat_key=target_chat_key,
            preset_key=target_preset_key,
            bot_self_introl=bot_self_introl,
        ):
            return {
                "msg": f"编辑预设: {target_preset_key} (￣▽￣)-ok! (会话: {target_chat_key})",
                "is_progress": True,
            }
        else:
            return {
                "msg": f"编辑预设: {target_preset_key} (会话: {target_chat_key}) 错误 ＞﹏＜!",
                "is_progress": True,
            }
    else:  # 当前会话应用
        if PersistentDataManager.instance.update_preset(
...
def _(option_dict, param_dict, chat: Chat, chat_presets_dict: dict):
    ...
    if option_dict.get("global"):  # 全局应用
        if option_dict.get("global"):  # 全局应用
            success_cnt, fail_cnt = PersistentDataManager.instance.del_preset_for_all(
                preset_key=target_preset_key
            )
            return {
                "msg": f"删除预设: {target_preset_key} (￣▽￣)-ok! (所有会话) 成功:{success_cnt}，失败:{fail_cnt}",
                "is_progress": True,
            }
        target_chat_key = option_dict.get("target")
        if target_chat_key not in global_chat_dict:
            return {"msg": f"会话: {target_chat_key} 不存在! (；′⌒`)"}
        if PersistentDataManager.instance.del_preset(
            chat_key=target_chat_key, preset_key=target_preset_key
        ):
            return {
                "msg": f"删除预设: {target_preset_key} (￣▽￣)-ok! (会话: {target_chat_key})",
                "is_progress": True,
            }
        else:
            return {
                "msg": f"删除预设: {target_preset_key} (会话: {target_chat_key}) 错误 ＞﹏＜!",
                "is_progress": True,
            }
    else:  # 当前会话应用
        if PersistentDataManager.instance.del_preset(
            chat_key=chat.get_chat_key(), preset_key=target_preset_key
        ):
...
def _(option_dict, param_dict, chat: Chat, chat_presets_dict: dict):
    if option_dict.get("global"):  # 全局应用
        if option_dict.get("global"):  # 全局应用
            success_cnt, fail_cnt = PersistentDataManager.instance.reset_chat_for_all()
            return {
                "msg": f"重置会话(￣▽￣)-ok! (所有会话) 成功:{success_cnt}，失败:{fail_cnt}",
                "is_progress": True,
            }
    elif option_dict.get("target"):  # 指定会话应用
        target_chat_key = option_dict.get("target")
        if target_chat_key not in global_chat_dict:
            return {"msg": f"会话: {target_chat_key} 不存在! (；′⌒`)"}
        if PersistentDataManager.instance.reset_chat(chat_key=target_chat_key):
            return {"msg": f"重置 (￣▽￣)-ok! (会话: {target_chat_key})", "is_progress": True}
        else:
            return {"msg": f"重置 (会话: {target_chat_key}) 错误 ＞﹏＜!", "is_progress": True}
    else:  # 当前会话应用
        if PersistentDataManager.instance.reset_chat(chat_key=chat.get_chat_key()):
