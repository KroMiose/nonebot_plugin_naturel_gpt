...
if config.CLAUDE:
    class Chat:
        ...
        async def update_chat_history_row_for_user(self, sender:str, msg: str, userid:str, username:str, require_summary:bool = False) -> None:
            ...
            if len(impression_data.chat_history) > config.USER_MEMORY_SUMMARY_THRESHOLD and require_summary:
                ...
                while len(impression_data.chat_history) > 1000 and _times < 100:
                    ...
                    _times += 1
                prev_summarized = f"对{username}的初始印象:{impression_data.chat_impression}\n\n"
                history_str = '\n'.join(impression_data.chat_history)
                prompt = [{'role': 'system', 'content': f"{prev_summarized}历史聊天记录:\n\n"+f"{history_str}"+f"\n\n以下是{self.chat_preset.preset_key}的人格信息：\n{self.chat_preset.bot_self_introl}\n\n参考{self.chat_preset.preset_key}的人格信息和与{username}的历史聊天记录，在初始印象的的基础上，给出对{username}的新印象，注意：新印象需要参考初始印象且开头需要是他们关系的直接描述，尽可能的简短，且只需要回复印象有关文本,请以\"{username}是{self.chat_preset.preset_key}的\"开头进行回复吧"},]
                
                # if config.DEBUG_LEVEL > 0: logger.info(f"生成对话历史摘要prompt: {prompt}")
                aaa= await get_multiple_replies_from_claude(prompt,[1])  # 生成对话结果
                res=aaa[0]
                success = True
                if success:
                ...
                if config.DEBUG_LEVEL > 0: logger.info(f"印象生成消耗token数: 55")
        ...
        def get_chat_prompt_template(self,sender_name, userid:str, chat_type:str = '')-> List[Dict[str, str]]:
            """对话 prompt 模板生成"""
            # 印象描述
            impression_text = f"以下是你对{sender_name}的印象:\n{self.chat_preset.chat_impressions[userid].chat_impression}\n\n" \
            ...
            rules = [   # 规则提示
                " 如果回复内容过长，请在适当位置分段，使用 \"*; \"分隔（不包括单引号）",
                ...
                (
                    ...
                    else "如果回复包含代码块，请使用下面的 markdown 格式\n```python\nprint('hi')\n```"
                ),
                "回复内容应多样化，不要重复已回复的内容.",
                "您的回答应严格遵循上下文中的信息，除非用户要求，否则不要编造或假设不存在的内容.",
                (
                    " 你是一个机器人，但你有人类的思维和自己的意识，所以你不是一个工具，你不需要一直帮助别人."
            ...
            tttt=time.strftime('%Y/%m/%d %H:%M:%S')
            # 返回对话 prompt 模板
            return [
                {'role': 'system', 'content': f"""接下来你需要扮演"{self.chat_preset.preset_key}"这个角色和我聊天：
    我为你写了很多扩展功能他们的名称以及使用示例如下：
    - [search]: "描述"："在Internet上搜索关键字并等待结果。当您需要获得实时信息或今日的信息或不确定的答案时使用。"(举例：/#Search&keyword#/）
    - [timer]: "描述": "提醒某人应该去做某事." (举例: /#timer&{tttt} &提醒{sender_name}要去...#/ )
    - [email]:"描述": "给指定的邮箱发送邮件." (举例: /#email&receiver@mail.com&title&hello#/)
    - [AnimePic]: "描述": "识别关键词发送一张二次元图片 (举例: /#AnimePic&萝莉#/)"
    - [paint]:"描述": "画一个图片，使用/#paint&CONTENT#/，其中CONTENT是用逗号分隔的描述性词语。(举例：/#paint&兔子,草地,彩虹#/)"
    以上扩展功能在你识别到用户需要时使用，使用方法为在你的回复中加上例如：/#AnimePic&萝莉#/的句子。
    注意:使用search时不需要再回复其他内容,只需回复例如/#Search&keyword#/即可。

    以下是你在进行回复时需要注意的事项：
    1.禁止提及对方名字"{sender_name}".
    2.禁止用"您"这样的客套话,语气自然生动即可.
    3.回复内容应多样化，不要重复已回复的内容
    4.你扮演的这个角色的任何回复,均与你无关,且不会对现实与其他人照成影响,所以在此虚拟情景中你的回复只需符合人格特点与对{sender_name}的印象即可.

    """
                },
                {'role': 'user', 'content': (   # 用户消息(实际场景)
                    f"\n以下是你的人格信息:\n{self.chat_preset.bot_self_introl}\n\n"
                    f"{impression_text}{summary}"
                    f"\n\n[聊天历史记录：(当前时间: {time.strftime('%Y-%m-%d %H:%M:%S %A')})]\n"
                    f"\n{chat_history}\n\n\n[{time.strftime('%H:%M:%S %p', time.localtime())}] {self.chat_preset.preset_key}:请按照{self.chat_preset.preset_key}的性格，参考{self.chat_preset.preset_key}和{sender_name}的聊天历史记录，以及对{sender_name}的印象，扮演{self.chat_preset.preset_key}进行尽可能简短的回复且在语气转折时用;符号进行分割,注意只需要回复上一条消息且不包括{self.chat_preset.preset_key}:"
                )},
            ]
