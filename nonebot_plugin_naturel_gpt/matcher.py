...
from nonebot.params import CommandArg, Event, Matcher
...
""" ======== 注册消息响应器 ======== """
# 注册消息响应器 收到任意消息时触发
matcher: Matcher = on_message(
    priority=config.NG_MSG_PRIORITY, block=config.NG_BLOCK_OTHERS
)
...
async def handler(matcher_: Matcher, event: MessageEvent, bot: Bot) -> None:
    ...
    # 处理消息前先检查权限
    (permit_success, _) = await permission_check_func(
        matcher=matcher_, event=event, bot=bot, cmd=None, type="message"
    )
...
async def _(
    matcher_: Matcher, event: GroupIncreaseNoticeEvent, bot: Bot
):  # event: GroupIncreaseNoticeEvent  群成员增加事件
    ...
    # 处理通知前先检查权限
    (permit_success, _) = await permission_check_func(
        matcher=matcher_, event=event, bot=bot, cmd=None, type="notice"
    )
    if not permit_success:
        return
    ...
    if isinstance(event, GroupIncreaseNoticeEvent):  # 群成员增加通知
        ...
        if config.DEBUG_LEVEL > 0:
            logger.info(f"未知通知来源: {event.get_session_id()} 跳过处理...")
        return
    ...
    user_name = (
        await get_user_name(event=event, bot=bot, user_id=event.get_user_id())
        or f"qq:{event.get_user_id()}"
    )
...
""" ======== 注册指令响应器 ======== """
# 人格设定指令 用于设定人格的相关参数
identity: Matcher = on_command(
    "identity",
    aliases={"人格设定", "人格", "rg"},
    rule=to_me(),
    priority=config.NG_MSG_PRIORITY - 1,
    block=True,
)
...
@identity.handle()
async def _(
    matcher_: Matcher, event: MessageEvent, bot: Bot, arg: Message = CommandArg()
):
    ...
    # 执行命令前先检查权限
    (permit_success, permit_msg) = await permission_check_func(
        matcher=matcher_, event=event, bot=bot, cmd=raw_cmd, type="cmd"
    )
    if not permit_success:
    ...
    if not raw_cmd:
        ...
        is_progress = True
        await identity.finish(
            f"预设: {target_preset_key} | 人设信息:\n    {chat_presets_dict[target_preset_key].bot_self_introl}"
        )

    elif (raw_cmd.split(" ")[0] in ["更新", "update", "edit"]) and len(
        raw_cmd.split(" ")
    ) >= 3:
        target_preset_key = raw_cmd.split(" ")[1]
        if target_preset_key not in chat_presets_dict:
            await identity.finish("找不到匹配的人格预设! 是不是手滑了呢？(；′⌒`)")
        if chat_presets_dict[target_preset_key].is_locked and (
            str(event.user_id) not in config.ADMIN_USERID
        ):
        ...
        PersistentDataManager.instance.add_preset(
            chat_key=chat_key, preset_key=target_preset_key, bot_self_introl=introl
        )
        is_progress = True
        await identity.send(f"添加预设: {target_preset_key} 成功! (￣▽￣)")

    elif (raw_cmd.split(" ")[0] in ["删除", "del", "delete"]) and len(
        raw_cmd.split(" ")
    ) == 2:
        target_preset_key = raw_cmd.split(" ")[1]
        # if str(event.user_id) not in config.ADMIN_USERID:
        ...
        await identity.finish((f"当前已加载的扩展模块:\n{ext_info}"))
        ...
        if not (
            global_extensions.get("remember")
            and global_extensions.get("forget")
            and config.MEMORY_ACTIVE
        ):
...
async def do_msg_response(
    ...
    # 其它人格唤醒判断
    if (
        chat.get_chat_preset_key().lower() not in trigger_text.lower()
        and chat.enable_auto_switch_identity
    ):
    ...
    if chat.get_chat_preset_key() != current_preset_key:
        logger.warning(
            f"等待OpenAI请求返回的过程中人格预设由[{current_preset_key}]切换为[{chat.get_chat_preset_key()}],当前消息不再继续响应.1"
        )
        return
    ...
    if chat.get_chat_preset_key() != current_preset_key:
        logger.warning(
            f"等待OpenAI响应返回的过程中人格预设由[{current_preset_key}]切换为[{chat.get_chat_preset_key()}],当前消息不再继续处理.2"
        )
        return
