...
from typing import Dict, List, Set, Tuple, overload
...
class PersistentDataManager(Singleton["PersistentDataManager"]):
    ...
    def load_from_file(self, file_path: str):
        ...
        for v in self._datas.values():
            if not hasattr(v, "chat_history"):
                ...
                if preset_data:
                    preset_chat_history: List[str] = getattr(
                        preset_data, "chat_history"
                    )
    ...
    def add_preset_from_config(
        self, chat_key: str, preset_key: str, preset_config: PresetConfig
    ) -> bool:
    ...
    def update_preset(
        self, chat_key: str, preset_key: str, bot_self_introl: str
    ) -> bool:
        """修改指定chat_key人格预设"""
        presets = self.get_presets(chat_key)
    ...
    def del_preset(self, chat_key: str, preset_key: str) -> bool:
        ...
        if (
            chat_data.active_preset == preset_key
        ):  # 当前正在使用的预设不允许删除(后续可以改成先让Chat强制更改为默认预设再删除)
            return False
